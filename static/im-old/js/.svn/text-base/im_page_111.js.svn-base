	...
	*/
	...
	var showChatUI = function() {debugger
		$('#content').css({
			"display" : "block"
		});
	...
	var handleOpen = function(conn) {debugger
		//从连接中获取到当前的登录人注册帐号名
	curUserId = conn.context.userId;
	
	$.ajax({
		type:"get",
		url:"http://192.168.1.11:8080/product-web/chat/getContact",
		data: "signId="+GetQueryString("userId"),
		success: function(msg){debugger
			showChatUI();
			var data = JSON.parse(msg)
	       console.log( data.result );  
	       buildContactDiv("contractlist", data.result);//联系人列表页面处理
		},
...
/*			success : function(roster) {debugger
				...
				if (bothRoster.length > 0) {
					...
					//console.log(JSON.stringify(bothRoster))
	...
	//构造联系人列表
	var buildContactDiv = function(contactlistDivId, roster) {debugger
