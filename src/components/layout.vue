<template>
  <div class="layout-box">
    ...
    <div class="im-img pointer" v-if="showChat">
    	<!--<img src="../assets/images/about-us/gywm-img2.jpg" style="width: 50px; height: 50px;" @click="imChat"/>-->
    	<i class="layui-icon layui-icon-login-wechat" @click="imChat" title="点击聊天"></i> 
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
  	imChat(){
  		//window.open('../../static/im-old/index.html?userName='+this.user_name+'&userId='+this.user_id+'&type=1')
  ...
  mounted (){
  		let user_id = this.common.SStorage.getItem("userId")
  		let role_sn = this.common.LStorage.getItem("userPageInfo") ? this.common.LStorage.getItem("userPageInfo").roleSn : null		
  	
  		this.user_name = this.common.LStorage.getItem("userPageInfo") ? this.common.LStorage.getItem("userPageInfo").userName : ''
  		this.user_id = this.common.LStorage.getItem("userPageInfo") ? this.common.LStorage.getItem("userPageInfo").userId : null
  		...
  		if(user_id && (role_sn == '10002' || role_sn == '10005' || role_sn == '10008' || role_sn == '10013') && nowHref.indexOf('yjw_admin') > -1){
  			this.showChat = true
  		} 
...
.im-img{
	display: inline-block; position: fixed; bottom: 10px; right: 10px;
}
...
.layui-icon-login-wechat{
	font-size: 50px; color: #999;
}
.layui-icon-login-wechat:hover{
	color: #1E9FFF;
}
