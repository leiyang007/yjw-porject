  			...
  			<form class="layui-form" action="">
					  <div class="layui-form-item">
					    ...
					    <div class="layui-input-block">
					      <input type="text" name="username" v-model="username" required lay-verify="required|phone_main|usePwdCheck" placeholder="请输入用户名(手机号)" autocomplete="off" class="layui-input">
					    </div>
					  </div>
					  <div class="layui-form-item">
					  ...
					  <div class="layui-form-item">
					    <div class="">
					      ...
					      <button class="layui-btn login-btn" type="button" lay-submit lay-filter="login" @click="loginBtn">登录</button>
					    </div>
					  </div>
				</form>
  		</div>
					  ...
					  .then((res) => {
					  	...
					  	if(nowData.code == 0){
					  		...
					  		if(_this.usernamePwd && _this.veriCodeCheck()){
								...
								_this.common.SStorage.setItem("author", nowData.result.author)		/*保存权限菜单信息*/
...
<style type="text/css">
	.placeholder{
		position: absolute; top: 6px; left: 10px; color: #C2BFBF;
	}
</style>
<style scoped>
	.login-dialog{
	...
	.login-dialog .layui-input-inline{
		position: relative;
	}
	.login-btn{
