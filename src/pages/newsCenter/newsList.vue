		...
		<div class="cont-bot-box">			
			<ul class="news-ul">				
				<li v-for="item in articleList">
					...
					<div class="new-li-box float-right">
						...
						<div class="news-brief float-right">
							<router-link :to="{path: '/newsInfo#here',query:{ 
								...
								<p class="line-1 article-title" @click="returnBack" :title="item.title">{{ item.title }}</p>
							</router-link>
							<p class="line-3 article-cont" style="height: 57px;" :title="item.introduction">{{ item.introduction }}</p>
						</div>	
					</div>									
				</li>
...
export default{
	...
	methods:{
			  ...
			  .then((res) => {
			  	...
			  	if(nowData.code == 0){
			  		...
			  		for(let i=0; i<_this.articleList.length; i++){
			  			...
			  			_this.articleList[i].introduction = _this.overflowhidden(introduction,3)
			  		}
				    ...
				    console.log(Math.ceil(_this.totalPage/_this.pageNumber))
		...
		overflowhidden (str, rows){
		     if(rows == 1){
	     		let sbu_str = this.subStringResult(str,48)
		     	return sbu_str
		     }else if(rows == 3){
	     		let sbu_str = this.subStringResult(str,172)
		     	return sbu_str
		     }		     
		}
	},
