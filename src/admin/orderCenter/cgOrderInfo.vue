	...
	<div class="cg-order-info">
		<div class="content-nav">
			<ul>
				...
				<li class="blue-font">采购订单详情</li>
			</ul>
		</div>
		<div class="reg-step-icon clear">
			<ul class="clear">
				<li :class="{'now-stap': status == 0 || status == 1 || status == 2 || status == 3 || status == 4 || status == 5 }">
					<div>1</div>
					<p>提交订单</p>
				</li>
				<li :class="{'now-stap': status == 2 || status == 3 || status == 4 || status == 5}">
					<div>2</div>
					<p>审核订单</p>
				</li>
				<li :class="{'now-stap': status == 3 || status == 4 || status == 5}">
					<div>3</div>
					<p>待出库</p>
				</li>
				<li :class="{'now-stap': status == 4 || status == 5}">
					<div>4</div>
					<p>配送中</p>
				</li>
				<li :class="{'now-stap': status == 5}">
					<div>5</div>
					<p>已签收</p>
				</li>
			</ul>
		...
		<div class="clear" style="padding: 20px 0 20px;">
			...
			<div class="float-right" v-show="isShowShenheBut == true || status2 == 1">
		...
		<div class="table-head clear">
			...
			<el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
			  <el-form-item label="单据编号" prop="djbh">
			    <el-input v-model="orderId" style="width:200px"  disabled></el-input>
			  </el-form-item>
			  <el-form-item label="供应商" prop="gys">
			    <el-input v-model="companyName" style="width:200px"  disabled></el-input>
			  </el-form-item>
			  <el-form-item label="业务员" prop="ywy">
			    <!--<el-input v-model="name" style="width:150px"></el-input>-->
			      <el-select v-model="ywy_name" :data-seller="sellerUserId" filterable placeholder="请选择" @change="onSelectedDrug($event, item)" :disabled="status != 0">
				    <el-option
		...
		<div style="margin-top: 20px;">
			...
			<div class="" v-show="isShowShenheBut == true || status2 == 1">
				<el-button type="warning" @click="shenheSubmit">审核</el-button>
			</div>
		</div>
		<!--提交弹窗-->
		<el-dialog title="提交" :visible.sync="dialogFormVisible">
		  <el-form :model="form">
		    <el-form-item label="下一个审批人" :label-width="formLabelWidth" v-if="auditStatus != 2">
		      <el-select v-model="form.shenpiPerson" @change="choseShenpiren2" placeholder="请选择">
		        <el-option
		...
		<!--审核弹窗2-->
		<el-dialog title="审核" :visible.sync="dialogFormVisible2">
		  <el-form :model="form2">
		  	<el-form-item label="下一个审批人" :label-width="formLabelWidth" v-if="(statusType == 1 && auditStatus != 2) || (statusType == 0 && auditStatus != 1)">
		      <el-select v-model="form2.shenpiPerson" @change="choseShenpiren" placeholder="请选择">
		        <el-option
			      v-for="item in shenpiNameList"
			      :key="item.id"
			      :label="item.name"
...
export default{
	data(){
		return{	
			...
			status: this.$route.query.status,			//订单状态
			companyName: this.$route.query.companyName,	//卖家或买家公司名
			ywy_name: '',			//卖家或买家业务员id
			sellerUserId: this.$route.query.sellerUserId,		//经销商时页面进来已选好默认传的id
			...
			auditStatus: this.$route.query.auditStatus? this.$route.query.auditStatus : null,			//审核进程状态	
	      	...
	      	provinceList: [],      //省列表 	 
	      	 cityList: [],      	//市列表 
	      	 areaList: [],      	 //区列表   
	      	 areaNumber: this.$route.query.locationId,		//地区编号
	...
	methods:{
    	...
    	confirm2(){
    		...
    		let ywy_name = ''
    		for(let i=0; i<this.buyerUserId.length; i++){
    			if(this.buyerUserId[i].userId == this.ywy_name){
    				ywy_name = this.buyerUserId[i].name
    			}
    		}
    		
    		//查询下一个审批人名称和ID
    		let this_juese = this.juese2
    		let this_name = null
    		...
    		console.log(JSON.stringify(this.juese2+this_name))
    		...
    		let parm = {
						 ...
						 "auditType": this.common.SStorage.getItem("saveUserInfo").userType,
					   "auditStatus": this.form2.shenheResult,
					  "sellerUserId": this.sellerUserId,
					"sellerUserName": this.ywy_name,
						   "auditor": this_juese,
					   "auditorName": this_name,
							"remark": this.form2.textarea,
		...
		choseShenpiren(){
			...
			let parm = {
				...
				"roleSn": this.form2.shenpiPerson,
		  		"signId": this.GLOBAL.userid
		  	}
  			console.log(parm)
				let baseparm = this.common.DataToBase64(JSON.stringify(parm))
		...
		shenheSubmit(){
			console.log("是否显示下一个审批人:"+this.auditStatus)
			/*if(this.common.SStorage.getItem("saveUserInfo").userType == 1){
				this.dialogFormVisible = true 
			}else{
		...
		savePageData(){
    		...
    		for(let i=0; i<this.buyerUserId.length; i++){
    			if(this.buyerUserId[i].name == this.ywy_name){
    				this.ywy_name = this.buyerUserId[i].userId
    			}
    		}
			...
			let param = {
				"order":{
					...
					"sellerUserId": this.ywy_name,
					//"sellerUserId": this.common.SStorage.getItem("saveUserInfo").userType == 0? this.sellerUserId : this.ywy_name,
		...
		queryBuyerInfo(cgy_id){		//查询买家信息
			let parm = {
		  		     "userId": cgy_id ? cgy_id : this.$route.query.buyerUserId,
		  			 "signId": this.GLOBAL.userid
		  	}
			console.log(JSON.stringify(parm))
			  ...
			  .then((res) => {				  	
			  	...
			  	if(nowData.code == 0){	
			  		console.log(JSON.stringify(nowData.result))
					this.cgy_name = nowData.result.name			//赋值采购员
			  	}else{
			  		console.log(nowData.message)
			  		this.$message({
			          message: nowData.message,
			...
			console.log(JSON.stringify(parm))
			  ...
			  .then((res) => {				  	
			  	...
			  	if(nowData.code == 0){	
					...
					this.sellerUserId = nowData.result.userOrderVo.sellerUserId
					//this.cgy_name = nowData.result.userOrderVo.buyerUserId  
					...
					this.queryBuyerInfo(this.cgy_id)	//找采购员
		...
		checkIsShenhe(){
			let auditype = this.common.SStorage.getItem("saveUserInfo").userType
			
			let parm = {
		  		    "orderId": this.$route.query.orderId,
		  		  "auditType": auditype == 0 ? '0' : '1',
		  		  	 "userId": this.common.SStorage.getItem("saveUserInfo").userId,
		  			 "signId": this.GLOBAL.userid
		  	}
			console.log(JSON.stringify(parm))
	...
	mounted(){	
		this.statusType = this.common.SStorage.getItem("saveUserInfo").userType
		console.log('判断是否有下一个审批人'+this.auditStatus)
		...
		if(this.taskId){
			...
			console.log(this.isShowShenheBut)
			console.log(this.status)
			...
			this.ywy_name = this.$route.query.name		//业务员反选必须传name才能显示姓名
			console.log(this.ywy_name)
			//isYiliao: false,		//是否是医疗机构
	      	 //isJingxiaoshang: false,		//是否是经销商
 
			if(this.isYiliao){
				this.getBuyerUserId(this.$route.query.sellerId,"10005",1)	//查询业务员
