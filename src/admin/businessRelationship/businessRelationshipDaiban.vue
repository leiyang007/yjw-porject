	    ...
	    getTableData(){
			...
			console.log(JSON.stringify(parm))
			  ...
			  .then((res) => {				  	
			  	...
			  	console.log(nowData)
			  	if(nowData.code == 0){				  		
					_this.tableData = nowData.result
					
					_this.imageUrl = nowData.result[0].appendix
					_this.status = nowData.result[0].status
					_this.formInline.buyerSn = nowData.result[0].buyerSn
					_this.formInline.cgdwName = nowData.result[0].buyerName
					_this.formInline.cgyName = nowData.result[0].buyerPeople
					_this.relationSn = nowData.result[0].relationSn
					_this.formInline.cgdwId = nowData.result[0].buyerId
					_this.formInline.cgyId = nowData.result[0].bpeopleId
					
					console.log('表格数组:--->'+JSON.stringify(_this.tableData))
