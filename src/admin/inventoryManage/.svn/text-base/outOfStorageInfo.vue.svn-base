		...
		<div class="table-head clear">
			...
			<el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
			  ...
			  <el-form-item label="采购单位" prop="gys">
			    ...
			    <el-select v-model="ruleForm.supplier" filterable placeholder="请选择" @change="caigouCompany(ruleForm.supplier)" >
				    <el-option
				      v-for="item in ruleForm.caigouCompanyList"
				      :key="item.id"
			  ...
			  <el-form-item label="关联的销售订单" prop="gys">
			    ...
			    <el-select v-model="ruleForm.unionCaigouOrder" filterable placeholder="请选择" @change="caigouOrderQuaryData(ruleForm.unionCaigouOrder)" >
				    <el-option
				      v-for="item in ruleForm.unionCaigouOrderList"
				      :key="item.orderId"
				      :label="item.orderId"
				      :value="item.orderId">
				    </el-option>
...
export default{
	...
	methods:{
		...
		getDriverList(){
 		...
 		getCaigouCompanyData(){
 			let _this = this
    		let parm = {
				   "sellerId": this.common.SStorage.getItem("saveUserInfo").companyId,
			     	"signId": this.GLOBAL.userid			     	
	    	}  
			...
			_this.$axios.get('/user_web/company/selectBuyerCompany?data='+baseParm+'&sign='+_this.GLOBAL.urlStr(parm))
			  .then((res) => {
			  	
			  	let nowData = JSON.parse(_this.common.base64ToData(res.data))
				console.log(JSON.stringify(nowData))
			  	if(nowData.code == 0){		
			  		_this.ruleForm.caigouCompanyList = nowData.result
			  		
			  		console.log(JSON.stringify(_this.ruleForm.caigouCompanyList))
			  		this.$message({
			           message: nowData.message,
			           type: 'success'
			        }) 
 		...
 		caigouCompany(vid){
			...
			_this.$axios.get('/user_web/userOrder/selectOrderForStockOut?data='+baseParm+'&sign='+_this.GLOBAL.urlStr(parm))
			  .then((res) => {
			  	
			  	let nowData = JSON.parse(_this.common.base64ToData(res.data))
				console.log(JSON.stringify(nowData))
			  	if(nowData.code == 0){		
			  		_this.ruleForm.unionCaigouOrderList = nowData.result
			  		
			  		console.log(JSON.stringify(_this.ruleForm.unionCaigouOrderList))
			  		this.$message({
			           message: nowData.message,
			           type: 'success'
			        }) 
	...
	mounted (){
		...
		this.getCaigouCompanyData()   //查询采购单位下拉列表数据
