	...
	<div class="qycg">
		...
		<div class="syetem-table">
			<div style="text-align: right; padding: 10px;">
				<span class="prompt-info" style="margin-left: 20px; float: left;">(点击订货按钮前<span class="red-font">必须填上数量</span>)</span>
			...
			<template>
			  <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%;text-align: left;" @selection-change="handleSelectionChange">
			    ...
			    <el-table-column prop="factoryName" label="生产厂家/销售单位">
			    </el-table-column>
			    <el-table-column prop="" label="联系企业">
			    	<template slot-scope="scope">
				    	<el-popover
						  placement="right"
						  width="100"
						  ...
						  <el-button slot="reference" @click="queryLianxiren(scope.row.companyId)" style="padding-left: 10px; padding-right: 10px;">选择联系人</el-button>
						</el-popover>
					</template>
			    </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
    	...
    	chatFor(id,name){
    		window.open(
    			'../../../static/im-old/index.html?otherUserName='+name+'&otherUserId='+id+'&userName='+this.common.SStorage.getItem("saveUserInfo").name+'&userId='+this.common.SStorage.getItem("userId")+'&type=2',
    			'_blank',
	    		...
	    		this_list = [{
						...
						"orderInfo":[{
						 ...
						 "registrationNumber": this.oneTableData.registrationNumber,
								 ...
								 "goodsPrice": this.oneTableData.price,
								 "totalPrice": this.oneTableData.goodsNum * this.oneTableData.price,
    			...
    			seleTableData[i] = {
		    			"order":{
								 ...
								 "totalPrice": selectArr[i].goodsNum * selectArr[i].prices,
								   "createId": this.common.SStorage.getItem("saveUserInfo").userId,
									"buyerId": this.common.SStorage.getItem("saveUserInfo").companyId,
								   "sellerId": selectArr[i].companyId,
								"buyerUserId": this.common.SStorage.getItem("saveUserInfo").userId,
							   "sellerUserId": ''
							   //"sellerUserId": selectArr[i].createId
						},
						"orderInfo":[{
									"goodsSn": selectArr[i].platformSn,
						 ...
						 "registrationNumber": selectArr[i].registrationNumber,
								   "goodsNum": selectArr[i].goodsNum ? selectArr[i].goodsNum : null,
								  "goodsUnit": selectArr[i].unit,
								 "goodsPrice": selectArr[i].price,
								 "totalPrice": selectArr[i].goodsNum * selectArr[i].price,
    	...
    	queryList(){
			...
			_this.$axios.post('/product-web/product/selectFactoryGoods?data='+baseParm+'&sign='+_this.GLOBAL.urlStr(parm))
			  .then((res) => {
		...
		xsdwOption(){
			...
			_this.$axios.post('/product-web/product/getFactoryName?data='+baseParm+'&sign='+_this.GLOBAL.urlStr(parm))
			  .then((res) => {
